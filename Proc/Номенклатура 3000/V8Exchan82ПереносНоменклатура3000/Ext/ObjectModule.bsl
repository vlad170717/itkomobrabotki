...
#Область РасширениеКонвертацииАдма

Функция адм_ВерсияРасширенияКонвертации() Экспорт
	Возврат "1.01.10";
	
КонецФункции

Функция адм_НаименованиеРасширенияКонвертации() Экспорт
	Возврат "Правила Остатки AA5 --> БухгалтерияПредприятия 3.0.74.69";
КонецФункции

Функция адм_ТекстЗапросаПо41Счету()
	Возврат "ВЫБРАТЬ 
	        |	ХозрасчетныйОстатки.Счет КАК Счет,
	        |	ХозрасчетныйОстатки.Субконто1.Код КАК Код,
	        |	ХозрасчетныйОстатки.Субконто1.Ссылка КАК Номенклатура,
	        |	ХозрасчетныйОстатки.Субконто1.Ссылка.Код КАК КодНом,
	        |	ХозрасчетныйОстатки.КоличествоОстатокДт КАК Количество,
	        |	ХозрасчетныйОстатки.Субконто2.Ссылка КАК Партия,
	        |	ХозрасчетныйОстатки.Субконто2.Ссылка.Дата КАК ПартияДата,
	        |	ХозрасчетныйОстатки.Субконто2.Ссылка.Номер КАК ПартияНомер,
	        |	ХозрасчетныйОстатки.Субконто2.Ссылка.Контрагент КАК Контрагент,
	        |	ХозрасчетныйОстатки.Субконто2.Ссылка.Контрагент.Наименование КАК КонтрагентНаименование,
...
Функция адм_СоздатьТЗОстатки()
	ТЗОстатки = Новый ТаблицаЗначений;
	
	ТЗОстатки.Колонки.Добавить("Количество");
	ТЗОстатки.Колонки.Добавить("Номенклатура");
	ТЗОстатки.Колонки.Добавить("Сумма");
	
	Возврат ТЗОстатки;
КонецФункции

Функция адм_ПолучитьОстаткиАА5КоторыхНет_в_БП_ИП()
	тзОстатки = адм_СоздатьТЗОстатки();
	
    тНом = ПрочитатьТЗИзMXL("C:\Amber\Номенклатура3000\СписокНомДляЗагрузкиВБП-ИП.mxl");

	тцЦенаЗакупки = Справочники.ТипыЦен.НайтиПоНаименованию("Закупочная");
	Для каждого стрНом Из тНом Цикл
		Ном = Справочники.Номенклатура.НайтиПоКоду(стрНом.Номенклатура);
		Количество = ПолучитьОстаток(Ном);
		Если Количество > 0 Тогда
			стрОст = тзОстатки.Добавить();
			стрОст.Номенклатура = Ном;
			стрОст.Количество = Количество;
			ВалютаЦенаЗакупки              = обВалютаТипаЦены(Ном, тцЦенаЗакупки);
			ЦенаЗакупки = обПолучитьЦену(тцЦенаЗакупки, Ном,,,ВалютаЦенаЗакупки);	
			стрОст.Сумма = ЦенаЗакупки * стрОст.Количество;
		КонецЕсли;
	КонецЦикла; 
...
Функция ПолучитьОстаток(Номенклатура)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |ИЗ
	               |	РегистрНакопления.ОстаткиТоваровКомпании.Остатки(&ВыбДата, Номенклатура = &Номенклатура) КАК ОстаткиТоваровКомпанииОстатки";
	Запрос.УстановитьПараметр("ВыбДата", Дата(2019, 12, 31, 23, 59, 59));
...
Функция адм_ВыгрузитьОстатки(тзОстатки)
	Строк = 0;
	Пачка = 1;
	МаксКоличествоСтрокТЧДокументаВводОстатков = 500;
	Организация = Справочники.Организации.НайтиПоКоду("00001"); //ИП Николаев
	
	ВходящиеДанные = адм_СоздатьСтруктуруВводОстатков();
	адм_ЗаполнитьШапкуВводаОстатков(ВходящиеДанные, Организация, Пачка);
	тчНоменклатураНаСкладе = адм_СоздатьТЗНоменклатураНаСкладе();
	...
	Для каждого стрОст Из тзОстатки Цикл
		...
		cтрокаНомНаСкладе = тчНоменклатураНаСкладе.Добавить();
		адм_ЗаполнитьСтрокуНомНаСкладе(cтрокаНомНаСкладе, стрОст, Организация);
		ИтогоКоличество = ИтогоКоличество + cтрокаНомНаСкладе.Количество;
		ИтогоСумма = ИтогоСумма + cтрокаНомНаСкладе.Сумма;

		
		Если Строк >= МаксКоличествоСтрокТЧДокументаВводОстатков Тогда 
			адм_ВыгрузитьВводОстатков(ВходящиеДанные, тчНоменклатураНаСкладе);
			Строк = 0;
			Пачка = Пачка + 1;
	...
	Если Строк > 0 Тогда
		адм_ВыгрузитьВводОстатков(ВходящиеДанные, тчНоменклатураНаСкладе);
	КонецЕсли; 
	Сообщить("Итого количество товаров (шт.):" + Формат(ИтогоКоличество, "ЧЦ="));
	Сообщить("Итого сумма :" + Формат(ИтогоСумма, "ЧЦ="));
КонецФункции

Процедура адм_ВыгрузитьВводОстатков(ВходящиеДанные, тчНоменклатураНаСкладе)
			ВходящиеДанные.Вставить("НоменклатураНаСкладе", тчНоменклатураНаСкладе);
			ВыгрузитьПоПравилу(,,ВходящиеДанные,,"ВводНачальныхОстатков");
КонецПроцедуры

Процедура адм_ЗаполнитьСтрокуНомНаСкладе(cтрокаНомНаСкладе, стрОст, Организация)
		cтрокаНомНаСкладе.Количество = стрОст.Количество;
		cтрокаНомНаСкладе.Номенклатура = стрОст.Номенклатура;
		cтрокаНомНаСкладе.Сумма = стрОст.Сумма;
		cтрокаНомНаСкладе.СчетУчета = "41.01";
КонецПроцедуры

Функция адм_ВыгрузитьДокументРасчетаСКонтрагентом(Дата, Номер, Организация, Контрагент, КонтрагентНаименование, НомерВхД, ДатаВхД)
	...
	ВходящиеДанные.Вставить("Комментарий", КонтрагентНаименование);
	ВходящиеДанные.Вставить("НомерВходящегоДокумента", НомерВхД);
	ВходящиеДанные.Вставить("ДатаВходящегоДокумента", ДатаВхД);
	
	ВыгрузитьПоПравилу(,,ВходящиеДанные,,"ДокументРасчетовСКонтрагентом");
	Возврат ВходящиеДанные;
КонецФункции

Функция адм_СоздатьТЗНоменклатураНаСкладе()
	ТЗНоменклатураНаСкладе = Новый ТаблицаЗначений;
	
	ТЗНоменклатураНаСкладе.Колонки.Добавить("Количество");
	ТЗНоменклатураНаСкладе.Колонки.Добавить("Номенклатура");
	ТЗНоменклатураНаСкладе.Колонки.Добавить("Склад");
	ТЗНоменклатураНаСкладе.Колонки.Добавить("СчетУчета");
	ТЗНоменклатураНаСкладе.Колонки.Добавить("Сумма");
	ТЗНоменклатураНаСкладе.Колонки.Добавить("Ключ");
	
	Возврат ТЗНоменклатураНаСкладе;
КонецФункции

Процедура адм_ЗаполнитьШапкуВводаОстатков(ВходящиеДанные, Организация, Пачка)
	ВходящиеДанные.Дата = адм_ДатаВремяПереноса();
	ВходящиеДанные.Номер = "ПА"+Формат(Пачка, "ЧЦ=5; ЧВН=; ЧГ=0; ЧФ=Ч");
	ВходящиеДанные.Организация = Организация;
	ВходящиеДанные.РазделУчета = "Товары";
	//ВходящиеДанные.Партии = "";
КонецПроцедуры

Функция адм_СоздатьСтруктуруВводОстатков()
	// перенести сюда вызов адм_ЗаполнитьШапкуВводаОстатков
	ВходящиеДанные = Новый Структура;
	
	ВходящиеДанные.Вставить("Дата", "");
	ВходящиеДанные.Вставить("Номер", "");
	ВходящиеДанные.Вставить("РазделУчета", "");
	ВходящиеДанные.Вставить("Организация", "");
	ВходящиеДанные.Вставить("ТестПараметра", "");
	
	Возврат ВходящиеДанные;
КонецФункции

Функция адм_ДатаВремяПереноса()
	Возврат КонецДня('20191231');
КонецФункции
